...
<span class="list-summary">By clicking this button, you acknowledge that you have read and agreed to Our <a href="<?php echo URL."page/?pages=Term-And-Conditions";?>" target="_blank">Term & Conditions</a> </span>
        ...
        <div class="reservation-vertical clearfix" id="reservationforms">
		  ...
		  <input type="hidden" name="prod" value="<?php echo $_GET["Hotel"];?>">
			...
			<li><span>Children</span><?php echo $_GET["child"];?> PAX<br>@ <?php echo  number_format($this->single[0]["Price"]);?> &nbsp; = &nbsp; <?php echo number_format($_GET["child"]*$this->single[0]["Price"]);?></li>
				...
				<input type="hidden" name="total" value="<?php echo (($_GET["adult"]*$this->single[0]["Price"])+($_GET["child"]*$this->single[0]["Price"]));?>">
				<input type="hidden" name="prod2" value="<?php echo $this->sts;?>">
			</ul>
			</div>
          </div>
        </div>
